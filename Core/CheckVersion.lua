...
local check = function(self, event, prefix, message, channel, sender)
	if event == "CHAT_MSG_ADDON" then
		if prefix ~= "FugarUIVersion" or sender == T.name then return end
		...
		if IsInGroup(LE_PARTY_CATEGORY_INSTANCE) then
			SendAddonMessage("FugarUIVersion", tonumber(T.version), "INSTANCE_CHAT")
		elseif IsInRaid(LE_PARTY_CATEGORY_HOME) then
			SendAddonMessage("FugarUIVersion", tonumber(T.version), "RAID")
		elseif IsInGroup(LE_PARTY_CATEGORY_HOME) then
			SendAddonMessage("FugarUIVersion", tonumber(T.version), "PARTY")
		elseif IsInGuild() then
			SendAddonMessage("FugarUIVersion", tonumber(T.version), "GUILD")
		end
	end
end
...
RegisterAddonMessagePrefix("FugarUIVersion")
...
whisp:SetScript("OnEvent", function(self, event, text, name, ...)
	if text:lower():match("ui_version") or text:lower():match("уи_версия") then
		if event == "CHAT_MSG_WHISPER" then
			SendChatMessage("FugarUI "..T.version, "WHISPER", nil, name)
		elseif event == "CHAT_MSG_BN_WHISPER" then
			BNSendWhisper(select(11, ...), "FugarUI "..T.version)
		end
	end
end)